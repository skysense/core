...
from model.model_helpers import stacked_lstm, ModelFileLogger, compute_returns, split_prices
...
class Model1(Model):
    ...
    def train(self, replay_file=os.path.join('data_examples', 'btc_price_2017-09-13T03:45:28+00:00.csv')):
        ...
        for i in range(self.steps):
            x_train, t_train, y_train = self.get_batch(self.batch_size, prices_train, self.sequence_length)
            st = time()
            self.sess.run([self.train_step], feed_dict={self.x_: x_train,
                                                        self.y_: y_train,
                                                        self.t_: t_train})  # gradient update.

            x_test, t_test, y_test = self.get_batch(self.batch_size, prices_cv, self.sequence_length)
